...
export const game = {
    ...
    elecFactor: 1.0,
    charts: {},
    currentFactIndex: null, // Speichert welchen Fakt wir anzeigen
    ...
    setLang: function(l) {
        ...
        this.updateTexts();
        if(this.step > 1) this.refreshCurrentPhase();
    },
    ...
    updateTexts: function() {
        ...
        // Dynamische Updates (Zertifikat, Summe etc)
        if(this.step === 3) this.updateSum();
        if(this.step === 5) this.updateSim(); 
    },

    refreshCurrentPhase: function() {
        if(this.step === 2) this.initPhase2();
        if(this.step === 3) this.initPhase3(); // Neu rendern für Textänderung
    ...
    nextStep: function(s) {
        document.querySelectorAll('.phase-container').forEach(e => e.classList.add('hidden'));
    ...
    initPhase2: function() {
        ...
        CAT_STRUCT.forEach(c => {
            ...
            el.className = 'sort-card'; el.dataset.phase = c.phase; el.dataset.id = c.id;
            el.innerHTML = `<i data-lucide="${c.icon}" size="14"></i> ${this.t('cats.'+c.id+'.l')}`;
            el.setAttribute('data-phase', c.phase); // Für CSS Styling
            src.appendChild(el);
            
            const lexItem = document.createElement('div');
            lexItem.className = 'lex-item';
            lexItem.innerHTML = `<strong>${this.t('cats.'+c.id+'.l')}:</strong> ${this.t('cats.'+c.id+'.s')}`;
            lex.appendChild(lexItem);
        });
        ...
        document.querySelectorAll('.drop-zone').forEach(z => z.style.background = "#fff");
    },
    ...
    checkSort: function() {
        ...
        zones.forEach(z => {
            const cards = document.getElementById('zone_'+z).querySelectorAll('.sort-card');
            cards.forEach(c => {
        ...
        if(wrong === 0 && correct === CAT_STRUCT.length) {
            fb.innerText = this.t('feedback_ok'); fb.style.background="#dcedc8"; fb.style.color="#2e7d32";
            setTimeout(() => this.nextStep(3), 1500);
        } else {
            fb.innerText = this.t('feedback_err'); fb.style.background="#ffcdd2"; fb.style.color="#c62828";
        }
    },
    ...
    initPhase3: function() {
        ...
        CAT_STRUCT.slice(0, 10).forEach(c => {
            ...
            row.innerHTML = `
                <label><i data-lucide="${c.icon}" size="16"></i> ${this.t('cats.'+c.id+'.l')}</label>
                <input type="range" class="guess" data-id="${c.id}" min="0" max="60" step="2" value="0">
                <span id="val_${c.id}" style="font-weight:bold; width:35px; text-align:right">0%</span>`;
            box.appendChild(row);
            row.querySelector('input').addEventListener('input', (e) => this.handleSlider(e.target));
        });
    ...
    initPhase4: function() {
        const user = Array.from(document.querySelectorAll('.guess')).map(i => parseInt(i.value) || 0);
        const real = DATA[this.sorte].values;
        const colors = CAT_STRUCT.slice(0,10).map(c => {
            if(c.phase === "Material") return "#558B2F";
            if(c.phase === "Herstellung") return "#8D6E63";
            if(c.phase === "Nutzung") return "#0288D1";
            return "#aaa";
        });
        ...
        CAT_STRUCT.forEach(c => {
            box.innerHTML += `<div class="lex-item"><strong>${this.t('cats.'+c.id+'.l')}:</strong> ${this.t('cats.'+c.id+'.d')}</div>`;
        });
        ...
        eol.innerHTML = `EoL: ${this.t('eol.'+eolKey)} <br> <span style="font-size:1.5em; font-weight:bold; color:${d.afterlife > 0 ? '#d32f2f' : '#388e3c'}">${d.afterlife > 0 ? '+' : ''}${d.afterlife}</span>`;
    },
    ...
    initPhase5: function() {
        ...
        MEASURE_DEFS.forEach(m => {
            ...
            btn.innerText = this.t('measures.'+m.id);
            btn.onclick = () => this.toggleMeasure(btn, m.id);
            box.appendChild(btn);
        });
        this.updateSim();
    },
    ...
    updateSim: function() {
        ...
        this.charts.sim = new Chart(document.getElementById('simChart'), {
            type: 'bar',
            data: {
                labels: [this.t('type_instant')||'Inst', this.t('type_filter')||'Filt', this.t('type_kapsel')||'Caps'],
                datasets: [{ 
                    label: 'kg CO₂e', 
                    data: [getKg('Instant'), getKg('Filter'), getKg('Kapsel')],
                    backgroundColor: ['#D7CCC8', '#8D6E63', '#4E342E']
                }]
    ...
    renderCertificate: function(savingPct) {
        ...
        let medal = "🥉 Bronze";
        let color = "#cd7f32";
        if(savingPct > 20) { medal = "🥈 Silver"; color = "#9E9E9E"; }
        if(savingPct > 50) { medal = "🥇 Gold"; color = "#FFC107"; }

        // Zufälligen Fakt holen (Index speichern)
        const facts = this.t('cert.facts');
        if(this.currentFactIndex === null) {
            this.currentFactIndex = Math.floor(Math.random() * facts.length);
        }
        const fact = facts[this.currentFactIndex] || facts[0];

        wrapper.innerHTML = `
            <div class="certificate" style="border-color: ${color}">
                <h3 class="cert-title">${this.t('cert.title')}</h3>
                <p>${this.t('cert.prefix')} <strong>${this.t('type_'+this.sorte.toLowerCase())}</strong></p>
                <div class="cert-val">${savingPct}% ${this.t('cert.reduced')}</div>
                <div class="cert-rank">${this.t('cert.rank')}: <span style="color:${color}; font-weight:bold;">${medal}</span></div>
